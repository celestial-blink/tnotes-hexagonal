...
export default component$<Props>(({ id, title, createdAt, isDraft, onClickItem$, onClickMenu$ }) => {
    ...
    return (
        <div class="flex flex-wrap justify-between border rounded gap-1 p-2 items-center bg-slate-100 dark:bg-slate-600 dark:text-white dark:border-none">
            <div class="flex gap-1 flex-1 h-full items-center justify-between">
                ...
                <div class="text-xs flex flex-col items-end flex-[.5]">
                    ...
                    <span class="block text-slate-500 mt-1 text-right dark:text-slate-200">Creado el {createdAt}</span>
                </div>
            </div>
            <details class="details relative" ref={refDetails}>
                ...
                <menu class="as-menu absolute bg-white p-1 right-0 rounded w-max flex flex-col dark:bg-slate-700" onClick$={handleClickMenuItem}>
                    <li>
                        <button data-action="update" class="text-right p-1 w-full hover:bg-slate-200 dark:hover:bg-slate-500">
                            Editar
                        </button>
                    </li>
                    <li>
